let promotionBox = document.getElementById("promotion-box");
let detailBox = document.getElementById("promotion-detail-box");
let detail = document.getElementById("promotion-detail");

function initApp(categoryId = "") {
  let promotionProducts = products.filter(
    (product) => product?.type == "promotion"
  );

  console.log(promotionProducts, "hh");

  promotionProducts.forEach((value, key) => {
    console.log(value);
    ...
    newDiv.innerHTML = `
        ...
        <div class="price promo-price">${value.normal_price.toLocaleString()} MMK</div>
    ...
    promotionBox.appendChild(newDiv);
  });
}
initApp();
